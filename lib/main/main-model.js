...
function MainModel(collection, Entity) {
  ...
  */
  ...
  /****Errors checking****/
  if(collection==null)
    throw ValmineError.getInstance('You must specify the collection to use in your current manager');
  if(Entity==null)
    throw ValmineError.getInstance('You must specify entity to use in your current manager');
  ...
  */
  this.findOne = function(id,callback) {
    if (!id || typeof id == 'function')
        throw ValmineError.getInstance(new TypeError(util.getParamErr('findOne','id')));
    if(util.isCallbackMissed(callback))
        throw ValmineError.getInstance(new TypeError(util.getParamErr('findOne','callback')));
  ...
  */
  this.findOneBy = function(filter,callback) {
    if (!filter || typeof filter=='function')
        throw ValmineError.getInstance(new TypeError(util.getParamErr('findOneBy','filter')));
    if(util.isCallbackMissed(callback))
        throw ValmineError.getInstance(new TypeError(util.getParamErr('findOneBy','callback')));
    db.collection(collection).findOne(filter,{}, function(err, doc) {
      if(err) console.log(err);
      if(doc !== null) {
        var entity = getParsedDoc(doc);
  ...
  **/
  this.findAll = function(callback) {
    if(util.isCallbackMissed(callback))
      throw ValmineError.getInstance(new TypeError(util.getParamErr('findAll','callback')));
  ...
  */
  this.findAllBy = function(filter, callback) {
    if(!filter || typeof filter == 'function')
      throw ValmineError.getInstance(new TypeError(util.getParamErr('findAllBy','filter')));
    if(util.isCallbackMissed(callback))
      throw ValmineError.getInstance(new TypeError(util.getParamErr('findAllBy','callback')));
    db.collection(collection).find(filter).toArray(function(err, docs) {
      if(err) console.log(err);
      if(docs.length > 0) {
        var objects = new Array();
  ...
  */
  var oParser = function(entity, callback) {
    if(util.isCallbackMissed(callback))
      throw ValmineError.getInstance(new TypeError(util.getParamErr('oParser','callback')));
    var data = {}
      , attribut = null
      , getter = null
      , objectId = null;
      ...
      if(/set/.test(method)) {
        ...
        if (getter!='getId') { /***Object id is null here so o.getId() will return null*/
          if(typeof entity[getter]=='undefined')
            throw ValmineError.getInstance('undefined method '+getter+', please checks the method definition in your class')
    ...
    if(util.isEmpty(data))
      throw ValmineError.getInstance('Setters of all objects must imperatively be preceded by the string "set"');
  ...
  */
  this.save = function(entity,callback) {
    if(!entity)
      throw ValmineError.getInstance(new TypeError(util.getParamErr('save','entity')));
    if(util.isEmpty(entity))
      throw ValmineError.getInstance('You are trying to save empty entity');
    if(util.isCallbackMissed(callback))
      throw ValmineError.getInstance(new TypeError(util.getParamErr('save','callback')));
  ...
  */
  this.remove = function(obj, callback) {
    if (!obj)
      throw ValmineError.getInstance(new TypeError(util.getParamErr('remove','object')));
    if (obj['getId'] !==null && typeof obj.getId === 'function')
      filter[idKey] = new ObjectID(obj.getId());
    else
      throw ValmineError.getInstance("The current object passed for delete does'nt have an getId method");
    if(util.isCallbackMissed(callback))
      throw ValmineError.getInstance(new TypeError(util.getParamErr('remove','callback')));
