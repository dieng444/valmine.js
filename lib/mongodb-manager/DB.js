...
function DB() {
    ...
    /****Errors checking***/
    if(port==null) throw ValmineError.getInstance('Database port can\'t be null, port = '+port);
    if(host==null) throw ValmineError.getInstance('Database host can\'t be null, host = '+host);
    if(name==null) throw ValmineError.getInstance('Database name can\'t be null, name = '+name);
